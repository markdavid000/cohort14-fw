...
const MOCK_HISTORY = [
  { event: 'Minted', value: 'Original Creation', address: '0x71C...a49B', time: '2 days ago', icon: 'mintmark', iconColor: 'text-secondary' },
  { event: 'Listed', value: null, address: '0x71C...a49B', time: '1 day ago', icon: 'list_alt', iconColor: 'text-primary-container' },
  { event: 'Offer Received', value: null, address: '0x12d...F45c', time: '4 hours ago', icon: 'swap_horiz', iconColor: 'text-tertiary' },
];

export function PropertyDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { properties, openModal, setSelectedPropertyId, isConnected, isOwner, addToast } = useApp();
  const mainRef = useRef<HTMLDivElement>(null);

  const property = properties.find((p) => p.id === Number(id));
  const { url: mainImage, location, apy } = property
  ...
  const statusBadge = property.isSold
    ...
    : property.isListed
    ? <Badge variant="minting" pulse>Live on Market</Badge>
    : <Badge variant="unlisted">Unlisted</Badge>;

  return (
    <div ref={mainRef} className="pt-20 md:pt-28 pb-16 md:pb-20 px-4 md:px-6 lg:px-12 max-w-[1600px] mx-auto page-enter">
          ...
          <div className="hidden md:grid col-span-4 grid-rows-2 gap-4">
            ...
            <div className="rounded-xl overflow-hidden relative group cursor-pointer">
              ...
              <div className="absolute inset-0 bg-surface-container-highest/40 flex items-center justify-center backdrop-blur-sm hover:bg-surface-container-highest/20 transition-all">
                <span className="font-headline font-bold text-xl">+ 8 Photos</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-7 lg:col-span-8 space-y-10 md:space-y-12">
          ...
          <section className="scroll-reveal">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
              {[
                { label: 'Asset Identifier', value: `OP-${String(property.id).padStart(4, '0')}-${property.category.slice(0, 2).toUpperCase()}` },
                { label: 'Market Valuation', value: formatPrice(property.price), highlight: true },
                { label: 'Asset Type', value: property.propType },
                { label: 'Category', value: property.category },
                { label: 'Owner Address', value: shortenAddress(property.owner) },
                { label: 'Status', value: property.isSold ? 'Sold' : property.isListed ? 'Listed' : 'Unlisted' },
              ].map((spec) => (
                <div key={spec.label} className="bg-surface-container-low p-5 md:p-6 rounded-xl">
                  <p className="text-on-surface-variant text-xs uppercase tracking-widest mb-2 font-label">
                    {spec.label}
                  </p>
                  <p className={`font-headline text-base md:text-lg font-bold ${spec.highlight ? 'text-primary-container' : 'text-on-surface'}`}>
                    {spec.value}
                  </p>
                </div>
              ))}
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-on-surface-variant text-sm leading-relaxed font-body">
                  {property.warranty}. This property is covered by Ocean Properties' structural
                  warranty, fully collateralized by the OP Governance Treasury. Includes
                  smart-contract-enforced maintenance escrow.
                </p>
              </div>
            </div>
          </section>
          ...
          {isOwner && (
            <section className="scroll-reveal">
              <h3 className="font-headline font-bold text-xl mb-4">Owner Actions</h3>
              <div className="flex flex-wrap gap-3">
                {!property.isSold && !property.isListed && (
                  <Button variant="primary" onClick={handleList}
                    icon={<span className="material-symbols-outlined text-base">storefront</span>}>
                    List Property
                  </Button>
                )}
                {!property.isSold && property.isListed && (
                  <Button variant="secondary" onClick={handleUnlist}
                    icon={<span className="material-symbols-outlined text-base">visibility_off</span>}>
                    Unlist Property
                  </Button>
                )}
                {!property.isSold && !property.isListed && (
        ...
        <aside className="md:col-span-5 lg:col-span-4 md:sticky md:top-28">
            ...
            <div className="space-y-5 md:space-y-6">
              ...
              <div className="bg-surface-container-lowest p-4 md:p-5 rounded-xl">
                <div className="flex justify-between items-center mb-3 md:mb-4">
                  ...
                  <span className="text-primary-container text-xs font-bold font-label">0.00 OP</span>
                </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-on-surface font-medium font-body text-sm">Approve Liquidity</span>
                </div>
                <div className="flex items-center gap-3 opacity-40">
                  ...
                  <span className="text-on-surface font-medium font-body text-sm">Execute Acquisition</span>
                </div>
              </div>
          ...
          <div className="mt-6 md:mt-8 flex justify-center gap-6 md:gap-8">
            <div className="text-center">
              <p className="text-xs text-on-surface-variant font-label uppercase mb-1">Est. APY</p>
              <p className="font-headline font-bold text-secondary">{apy}</p>
            </div>
            <div className="text-center">
              <p className="text-xs text-on-surface-variant font-label uppercase mb-1">Type</p>
              <p className="font-headline font-bold">{property.propType}</p>
            </div>
            <div className="text-center">
              <p className="text-xs text-on-surface-variant font-label uppercase mb-1">Status</p>
              <p className="font-headline font-bold">{property.isSold ? 'Sold' : property.isListed ? 'Live' : 'Vaulted'}</p>
            </div>
          </div>
        </aside>
      </div>
            ...
            <table className="w-full text-left border-collapse min-w-[500px]">
              <thead>
                <tr className="bg-surface-container text-on-surface-variant text-xs uppercase tracking-widest font-label">
                  <th className="px-4 md:px-8 py-4 md:py-5 font-semibold">Event Type</th>
                  <th className="px-4 md:px-8 py-4 md:py-5 font-semibold">Value</th>
                  <th className="px-4 md:px-8 py-4 md:py-5 font-semibold">Address</th>
                  <th className="px-4 md:px-8 py-4 md:py-5 text-right font-semibold">Time</th>
                </tr>
              </thead>
                ...
                {MOCK_HISTORY.map((row, i) => (
                  <tr key={i} className="hover:bg-surface-container-high transition-colors">
                    <td className="px-4 md:px-8 py-4 md:py-6">
                      <div className="flex items-center gap-3">
                        <span className={`material-symbols-outlined filled ${row.iconColor} text-xl`}>
                          {row.icon}
                        </span>
                        <span className="text-on-surface font-medium font-body text-sm">{row.event}</span>
                      </div>
                    </td>
